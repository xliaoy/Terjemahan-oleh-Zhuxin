...
| 🔍 **本地 OCR 识别** | 内置 ML Kit 本地文字识别，离线识别屏幕上的外文（拉丁语系） |
| 🌐 **在线 AI 翻译** | 接入任意 OpenAI 兼容接口（自定义 Base URL / API Key / 模型），自动拉取模型列表 |
| 📴 **离线本地翻译** | 内置腾讯混元 HY-MT1.5-1.8B 翻译模型 + llama.cpp 推理引擎，断网也能翻译 |
| 🖐 **手势操作** | 单击翻译 · 长按拖动窗口 · 上下滑动滚动译文 · 按住右下角手柄实时缩放识别区域 |
| 🫧 **悬浮球收纳** | 窗口可一键最小化为悬浮球，不遮挡内容，点按重新展开 |
| 🌍 **多语言支持** | 源语言/目标语言：自动检测、中、英、日、韩、法、德、西、俄 |
